...
def get_oe_ref(oe_0, t_0, horizon, n_time):
...
def check_ps_roe(states_roe, oe_vec, n_time, horizon_safe, n_safe, DEED):

    states_roe_ps = np.empty(shape=(6, n_time, n_safe), dtype=float)
    states_rtn_ps = np.empty(shape=(6, n_time, n_safe), dtype=float)
    min_sep_ps = np.empty(shape=(n_time, ), dtype=float)
    ...
    for k in range(n_time):
        sep_k = np.empty(shape=(1, n_safe), dtype=float)
        oe_j = oe_vec[:, k]
        state_roe_j = states_roe[:, k]
        state_rtn_j = map_roe_to_rtn(state_roe_j, oe_j)
        sep_k[:,0] = np.sqrt(np.transpose(state_rtn_j).dot(DEED.dot(state_rtn_j)))
        states_roe_ps[:,k,0] = state_roe_j
        states_rtn_ps[:,k,0] = state_rtn_j
        for j in range(n_safe-1):
            stm_j = state_transition_roe(oe_j, dt_safe)
            state_roe_j = stm_j.dot(state_roe_j)
            states_roe_ps[:,k,j+1] = state_roe_j
            oe_j = np.array([oe_j.item(0), oe_j.item(1), oe_j.item(2), oe_j.item(3), oe_j.item(4), oe_j.item(5) + n*dt_safe]).reshape((6,))      
            state_rtn_j = map_roe_to_rtn(state_roe_j, oe_j)
            states_rtn_ps[:,k,j+1] = state_rtn_j
            sep_k[:,j+1] = np.sqrt(np.transpose(state_rtn_j).dot(DEED.dot(state_rtn_j)))
        t_star_j = np.argmin(sep_k)
        min_sep_ps[k] = sep_k[:,t_star_j]
    
    return states_roe_ps, states_rtn_ps, min_sep_ps

def check_ps_constraint(states_roe, oe_vec, n_time, horizon_safe, n_safe, DEED):

    constr_ps = np.empty(shape=(n_time,), dtype=float)
    constr_ps_violation = np.zeros(shape=(n_time,), dtype=float)
    ...
    for k in range(n_time):
        constr_k = np.empty(shape=(1, n_safe), dtype=float)
        oe_j = oe_vec[:, k]
        state_roe_j   = states_roe[:, k]
        state_rtn_j   = map_roe_to_rtn(state_roe_j, oe_j)
        constr_k[:,0] = np.sqrt(np.transpose(state_rtn_j).dot(DEED[:,:,k].dot(state_rtn_j))) 
        
        # free-drift along the safe horizon
        for j in range(n_safe-1):
            stm_j = state_transition_roe(oe_j, dt_safe)
            state_roe_j = stm_j.dot(state_roe_j)
            oe_j = np.array([oe_j.item(0), oe_j.item(1), oe_j.item(2), oe_j.item(3), oe_j.item(4), oe_j.item(5) + n*dt_safe]).reshape((6,))      
            state_rtn_j = map_roe_to_rtn(state_roe_j, oe_j)
            constr_k[:,j+1] = np.sqrt(np.transpose(state_rtn_j).dot(DEED[:,:,k].dot(state_rtn_j)))
        t_star_j = np.argmin(constr_k)
        constr_ps[k] = constr_k[:,t_star_j]
        if (constr_ps[k] < 1):
            constr_ps_violation[k] = 1  # violation is 1 

    return constr_ps, constr_ps_violation

def check_ps_constraint_w_minsep(states_roe, oe_vec, n_time, horizon_safe, n_safe, DEED_ae, DEED_koz, DEED_sep, koz_sample_start):

    constr_ps = np.empty(shape=(n_time,), dtype=float)
    constr_ps_violation = np.zeros(shape=(n_time,), dtype=float)
    min_sep = np.empty(shape=(n_time,), dtype=float)
    ...
    for k in range(n_time):
        ...
        state_roe_j = states_roe[:, k]
        state_rtn_j = map_roe_to_rtn(state_roe_j, oe_j)
        if (k < koz_sample_start):
            constr_k[:,0] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_ae.dot(state_rtn_j)))
        else: 
            constr_k[:,0] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_koz.dot(state_rtn_j)))
        sep_k[:,0] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_sep.dot(state_rtn_j))) 
        for j in range(n_safe-1):
            stm_j = state_transition_roe(oe_j, dt_safe)
            state_roe_j = stm_j.dot(state_roe_j)
            oe_j = np.array([oe_j.item(0), oe_j.item(1), oe_j.item(2), oe_j.item(3), oe_j.item(4), oe_j.item(5) + n*dt_safe]).reshape((6,))      
            state_rtn_j = map_roe_to_rtn(state_roe_j, oe_j)
            if (k < koz_sample_start):
                constr_k[:,j+1] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_ae.dot(state_rtn_j)))
            else: 
                constr_k[:,j+1] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_koz.dot(state_rtn_j)))
            sep_k[:,j+1] = np.sqrt(np.transpose(state_rtn_j).dot(DEED_sep.dot(state_rtn_j))) 
        t_star_j = np.argmin(constr_k)
        constr_ps[k] = constr_k[:,t_star_j]
        min_sep[k] = sep_k[:,t_star_j]
        if (constr_ps[k] < 1):
            constr_ps_violation[k] = 1

    return constr_ps, constr_ps_violation, min_sep
...
def linearize_trans(oe, time):
    ...
    mats = {"stm_t": stm, "cim_t": cim, "psi": psi, "rt": rs}
