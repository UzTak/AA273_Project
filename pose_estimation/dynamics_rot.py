...
def qw_fwd(q,w,t0,tf,J,λ=0):
    I1, I2, I3 = J[0,0], J[1,1], J[2,2]
    H = J @ w
    T = 1/2 * w.T @ H 
    H = np.linalg.norm(H)  # we only need norm 
    
    ω1m = ((H**2 - 2*I3*T) / (I1*(I2-I3)))**0.5
    if H**2 >= 2*I2*T:
        ω2m = ((2*I2*T - H**2) / (I1*(I1-I3)))**0.5
    else: 
        ω2m = ((H**2 - 2*I3*T) / (I2*(I2-I3)))**0.5
    ω3m = ((2*I1*T - H**2) / (I3*(I1-I3)))**0.5

    # elliptic modulus 
    if H**2 >= 2*I2*T:
        k = ((I2-I3)*(2*I1*T-H**2) / ((I1-I2)*(H**2-2*I3*T)))**0.5
    else:
        k = ((I1-I2)*(H**2-2*I3*T) / ((I2-I3)*(2*I1*T-H**2)))**0.5
    ...
    t1 = t0 - 1/Ω * (sp.special.ellipkinc(Θ, k) + λ*K) 
    ...
    γ0 = ((H+Hn2) / (2*H)) ** 0.5
    γ1 = Hn3 * ((H - Hn2) / (2*H*(Hn1**2 + Hn3**2))) ** 0.5 
    γ2 = 0
    γ3 = - Hn1 * ((H-Hn2) / (2*H*(Hn1**2 + Hn3**2))) ** 0.5
    
    
    rot = np.array([[γ0,  γ1,  γ2,  γ3],
                   [-γ1,  γ0,  γ3, -γ2],
                   [-γ2, -γ3,  γ0,  γ1],
                   [-γ3,  γ2, -γ1,  γ0]])
    β0 = rot @ q
    
    β0_0, β1_0, β2_0, β3_0 = β0
    
    Φ0_0 = np.arctan2(β2_0, β0_0)
    Φ0_1 = np.arctan2(β3_0, β1_0)
    
    Φd0 = Φ0_0 - Φ0_1    
    Φs = np.arctan2(I1*ω1, -I3 * ω3)
    Φd = Φd0 + H/I2*(tf-t0) + (2*I2*T - H**2)/(I2*Ω*H) * float(elliptic_pi(a**2, k, τ) - elliptic_pi(a**2, k, τ0)) # TODO; IMCOMPLETE INTEGRAL THIRD KIND HERE  
    
    Φ0 = 1/2 * (Φs + Φd)
    Φ1 = 1/2 * (Φs - Φd) 
    β0 = (1/2*(1+a*sn)) ** 0.5 * np.cos(Φ0)
    β1 = (1/2*(1-a*sn)) ** 0.5 * np.cos(Φ1)
    β2 = (1/2*(1+a*sn)) ** 0.5 * np.sin(Φ0)
    β3 = (1/2*(1-a*sn)) ** 0.5 * np.sin(Φ1)
