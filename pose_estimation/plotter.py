...
def MRP_error_band(omega, true_omega, mrp, covariance, dt):
    ...
    for i in range(3):
        ...
        axs[i].set_title(rf'$\omega_{i+1}$', fontsize=12)
        ...
        axs2[i].set_title(rf'$\delta p_{i+1}$', fontsize=12)
        axs2[i].set_xlabel('Time (s)', fontsize=10)
        axs2[i].set_ylabel('Value', fontsize=10)
